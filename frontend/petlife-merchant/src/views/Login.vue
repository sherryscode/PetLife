    ...
    <div class="login-container">
      <div class="login-card">
        <div class="login-header">
          ...
          <p class="login-subtitle">欢迎回来，请登录您的商户账户</p>
        </div>
...
async function handleLogin() {
  ...
  loading.value = true
  try {
    const response = await axios.post('/auth/merchant/login', form)
    if (response.code === 200) {
      merchantStore.setToken(response.data.token)
      merchantStore.setMerchantInfo(response.data.merchant)
